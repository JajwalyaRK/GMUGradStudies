...
goods-own [
  ...
  matches                  ; list of the id of goods that each turtle has encountered
...
patches-own [
  ...
  gain-value               ;amount of resources that moving goods can gain
]
...
to setup-goods ;;turtle procedure
   create-goods number [ setxy 17 random-pycor
...
to setup-markets
  create-markets number_markets [
  setxy -17 random-pycor
  set color violet
  set shape "house"
  set size 2
  set demand random-normal Demand 2
  ]
end
...
to go
  ;if ticks >= 50 [stop]
  wait 0.5
  ask goods [move-goods]
  find-match
  gain-resources
  kill-goods
  update-globals
  ;if goods in-radius one-of market [stop]
  tick

end

to move-goods              ;consider moving to unoccupied patches towards nearest neighbours
  ;let dest-patches patches with [pcolor = black and not any? goods-here]
  ;ifelse any? dest-patches [
  ;  move-to min-one-of dest-patches [ distance patch 0 0 ]
  ask goods
  [
    right random 360
    forward 1
    ask patches in-radius 1
      [ set pcolor [brown] of myself ] ]
;; each good makes a colored "splotch" around itself
end

to find-match        ;consider matching with another turtle
  ask goods [ifelse matched? = false and any? goods in-radius 3 with [matched? = false] [Match] [move-goods]] ;if it has not been matched, it looks around to find goods not matched in radius 3. If they're all matched, the turtle moves.
  ask goods [ask my-links [ifelse [resource-level] of other-end <= 50 [die] [set color blue]]] ;
  ask goods [set num-links count my-links]
end

to Match              ;match with neighbouring turtle
  if any? other goods in-radius 3 [create-link-to one-of other goods in-radius 3] ;[move]; with [not matched?]
  set matches lput [who] of link-neighbors matches
  ask link-neighbors [set matched? true]
  exchange-resources
  ask goods with [matched? = true] [set matched? false]
end
...
to kill-goods
  if ticks mod 4 = 0 [
    ask goods [set age age + 1]
  ]
  ask goods with [age = 100] [hatch 1 set age 0 ask my-links [die]] ;give "birth" to child inheriting resources
  ask goods with [age > 100] [die]          ;consider dying if age is above maximum
  ask goods [if resource-level <= 0 [die]]  ;consider dying if resource level is below minimum
end

to gain-resources
  if ticks mod 4 = 0 [
  if random 100 < Gain_Likelihood [ ;the turtle is allowed to gain the resource on the patch
    ask goods [set resource-level resource-level + [gain-value] of patch-here]]
  ask goods [if resource-level >= 100 [set resource-level 100]] ;maximum resources that can be accumulated is set at 100
  ]
end
...
to update-globals
  ...
  set avg-high-resources mean [resource-level] of goods with [extraversion-level = "High"]
  set avg-med-resources  mean [resource-level] of goods with [extraversion-level = "Med"]
  set avg-low-resources  mean [resource-level] of goods with [extraversion-level = "Low"]

  ;sum total number of links
  set high-links sum [num-links] of goods with [extraversion-level = "High"]
  set med-links sum [num-links] of goods with [extraversion-level = "Med"]
  set low-links sum [num-links] of goods with [extraversion-level = "Low"]

  ;average number of links
  set avg-high-links mean  [num-links] of goods with  [extraversion-level = "High"]
  set avg-med-links  mean  [num-links] of goods with  [extraversion-level = "Med"]
  set avg-low-links  mean  [num-links] of goods with  [extraversion-level = "Low"]
end

;2019 Lindsay S. Shaffer, Jajwalya Karajgikar
